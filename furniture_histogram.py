"""
furniture_histogram.py
======================
Runs Sonata inference over every scene in `scannet_all_processed/` and:
  1. Builds a histogram: "what % of scenes contain each furniture class"
     (according to the model's predictions).
  2. Compares that histogram against the ground-truth `segment20.npy` labels.
  3. Prints and saves a side-by-side bar chart.

Usage
-----
    python furniture_histogram.py \
        --data_root  /home/obiwan/mirac/sonata/scannet_all_processed \
        --output_dir /home/obiwan/mirac/sonata/furniture_histogram_output \
        [--max_scenes N]          # limit to first N scenes for quick test
        [--min_point_frac 0.001]  # min fraction of points for a class to "count"
        [--device cuda]           # cuda | cpu

The `min_point_frac` threshold avoids noise-level detections being counted as
"class present": a class is considered present only if ≥ min_point_frac * N_pts
points are predicted as that class.
"""
...
# ── Model helpers (identical to inference_manual.py) ──────────────────────────
try:
    import flash_attn
    HAS_FLASH = True
except ImportError:
...
def infer_scene(point_raw, model, seg_head, transform, device):
    point = transform(dict(point_raw))           # shallow copy
    ...
    pred = seg_head(out.feat).argmax(dim=-1).cpu().numpy()   # (N,) int 0..19
    return pred


# ── Per-scene "class present" detection ───────────────────────────────────────
...
def main():
    ...
    for idx, scene_name in enumerate(all_scene_dirs):
        ...
        # ── Agreement (only where GT exists) ──────────────────────────────────
